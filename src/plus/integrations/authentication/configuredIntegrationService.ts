...
export class ConfiguredIntegrationService {
	...
	async getConfigured(options?: {
		...
		// Include a descriptor for the built-in VS Code session of GitHub even though we don't store it
		if ((options?.id == null || options.id === HostingIntegrationId.GitHub) && options?.type !== 'cloud') {
			...
			if (options?.id === HostingIntegrationId.GitHub && options?.type === 'local') {
				return descriptors;
			}
		}
	...
	private async addConfigured(descriptor: ConfiguredIntegrationDescriptor) {
	...
	private async removeConfigured(id: IntegrationId, options?: { domain?: string; type?: ConfiguredIntegrationType }) {
	...
	async storeSession(id: IntegrationId, session: ProviderAuthenticationSession) {
	...
	async deleteAllStoredSessions(id: IntegrationId, type?: ConfiguredIntegrationType) {
		await this.deleteAllSecrets(id, type);
	}

	async deleteSecrets(id: IntegrationId, sessionId: string, type?: ConfiguredIntegrationType) {
	...
	async deleteAllSecrets(id: IntegrationId, type?: ConfiguredIntegrationType) {
	...
	async writeSecret(id: IntegrationId, session: ProviderAuthenticationSession | StoredSession) {
